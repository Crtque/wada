...
<div style="width:32%;height:100%"> </div>
		...
		<?php 
			if($_SESSION['nick']){
					$flag = 0;
					$q = "SELECT id,hash FROM hashs ORDER BY id DESC LIMIT 0,1";
					$rq = mysqli_query($db, $q);
					$row = mysqli_fetch_object($rq);
					$id = $row->id;
					$hash = $row->hash;
					
					$i=1;
					
					$z = "SELECT COUNT(id) AS count FROM hashs";
					
					$rz = mysqli_query($db, $z);
					$rowz = mysqli_fetch_object($rz);
					$count = $rowz->count;
					while($i < $count){
					curl_close($ch);

					$ch = curl_init();
					curl_setopt($ch, CURLOPT_URL, "http://10.177.0.218:9191/storage/".$hash);
					...
					$qw = "SELECT id,hash FROM hashs ORDER BY id DESC LIMIT $i,".($i+1);
					$rqw = mysqli_query($db, $qw);
					$roww = mysqli_fetch_object($rqw);
					$hashw = $roww->hash;
					if($hashw == $prev){
							$hash = $hashw;
							$i++;
					...
					if($flag == 1){
						echo '<div class="container col-4 border rounded p-4 px-5 my-5 ml-5 shadow-sm vcenter" style="align-content:center; background: #343A87;color:red">
							<h1 class="text-center">ОБНАРУЖЕНО СТОРОННЕЕ ВМЕШАТЕЛЬСТВО В БД. ДАННЫМ НЕЛЬЗЯ ДОВЕРЯТЬ</h1>
							</div>';
					}else{
							echo '<div class="container col-4 border rounded p-4 px-5 my-5 ml-5 shadow-sm vcenter" style="align-content:center; background: #343A87;color:green">
							<h1 class="text-center">БД НЕ ИЗМЕНЕНА. НЕ ВЫЯВЛЕНО НЕДОПУСТИМЫХ ИСПРАВЛЕНИЙ ДАННЫХ. ДАННЫМ МОЖНО ДОВЕРЯТЬ</h1>
							</div>';
					}
